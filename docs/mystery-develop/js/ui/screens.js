...
export function showDevicePickup(renderer,{onPickup}){
  ...
  target.addEventListener('click',()=>{target.disabled=true;section.classList.add('is-picking-up');renderer.announce('端末を拾いました');const delay=globalThis.matchMedia?.('(prefers-reduced-motion: reduce)').matches?80:900;setTimeout(()=>onPickup(),delay)},{once:true});
}
